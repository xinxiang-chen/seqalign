...
def compute_last_row(X, Y):
    ...
    for i in range(1, m+1):
        ...
        for j in range(1, n+1):
            yj = Y[j-1]
            diag = prev[j-1] + mismatch_cost(xi, yj)
            up = prev[j] + DELTA
            left = curr[j-1] + DELTA
...
def hirschberg(X, Y):
    ...
    if m == 1 or n == 1:
        return full_dp_alignment(X, Y)

    mid = m // 2

    L = compute_last_row(X[:mid], Y)
    R = compute_last_row(X[mid:][::-1], Y[::-1])
    ...
    ax1, ay1 = hirschberg(X[:mid], Y[:best_k])
    ax2, ay2 = hirschberg(X[mid:], Y[best_k:])
...
def efficient_alignment(X, Y):
    ...
    total_cost = 0
    for c1, c2 in zip(ax, ay):
        if c1 == '_' or c2 == '_':
            total_cost += DELTA
        else:
            total_cost += mismatch_cost(c1, c2)
    return total_cost, ax, ay
...
def main():
    ...
    cost, ax, ay, t_ms, mem_kb = measure_algorithm(efficient_alignment, X, Y)
